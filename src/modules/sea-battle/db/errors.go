...
func (q SeaBattleQueries) SaveNewError(roomName string, errorMsg string) {
	...
	if err != nil {
		fmt.Printf("Error in SaveNewError. Error: %s\n", err.Error())
	}
}

func (q SeaBattleQueries) SaveAutoRoomDeletion(roomInfo DB_PlayerWithRoomJoinRow) {
	info := fmt.Sprintf("Room with ID %v deleted with data: player_email - %v, player_id - %v, room_name - %s.",
		roomInfo.RoomId,
	...
	_, err := q.db.Exec(`INSERT INTO seabattle_errors(error) VALUES($1);`, info)
	if err != nil {
		fmt.Printf("Error in SaveAutoRoomDeletion. Error: %s\n", err.Error())
	}
}
